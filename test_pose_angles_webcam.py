#!/usr/bin/env python3
"""
Host-Side Pose Estimation with REBA Scoring (NO OAK device required)
=====================================================================
Uses your laptop webcam and runs YOLOv8n-pose on CPU via ultralytics.
...
Run:
    ...
    python test_pose_angles_webcam.py --source 0
    python test_pose_angles_webcam.py --source video.mp4
    python test_pose_angles_webcam.py --csv reba.csv --video reba.mp4

REBA adjustment flags (applied globally to all frames):
    python test_pose_angles_webcam.py --neck-twist --trunk-side-bend
    python test_pose_angles_webcam.py --shoulder-raised --arm-abducted
...
def signed_angle_from_vertical(p_bottom, p_top):
    ...
    From a side-view camera, positive typically maps to forward flexion
    and negative to extension, but this depends on facing direction.
    """
...
# Keypoint extraction
# ---------------------------------------------------------------------------
def extract_points(kp_data):
    """Convert [17, 3] tensor (x_px, y_px, conf) to list of (x,y) or None."""
    pts = []
...
def draw_reba_table(frame, rb, fps):
    ...
    # Test mode label at bottom
    cv2.putText(
        frame, "[HOST CPU - TEST MODE]", (8, frame.shape[0] - 12),
...
def main():
    parser = argparse.ArgumentParser(
        description="Host-side REBA pose scoring (no OAK needed)")
    parser.add_argument(
        "--source", default="0",
    ...
    log("=" * 56)
    log("Host-Side REBA Pose Scoring")
    log("=" * 56)
    ...
    log("Loading model (downloads weights on first run)...")

    model = YOLO(MODEL_NAME)

    cap = cv2.VideoCapture(source)
    if not cap.isOpened():
        log(f"ERROR: Cannot open video source: {source}")
        sys.exit(1)
    ...
    vid_fps = src_fps if src_fps > 0 else 30.0
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    video_writer = cv2.VideoWriter(video_path, fourcc, vid_fps, (frame_w, frame_h))
    ...
    log("Running. Press 'q' in the OpenCV window to quit.")
    log("")
    ...
    try:
        while True:
            ret, frame = cap.read()
            if not ret:
                log("End of video or webcam read failed.")
                break

            results = model(frame, conf=DET_THRESHOLD, verbose=False)
            ...
            if len(results) > 0 and results[0].keypoints is not None:
                ...
                if kp_tensor.shape[0] > 0:
                    kp_data = kp_tensor[0]
                    pts = extract_points(kp_data)
                    rb = compute_reba(pts, flags)

            # Draw
            draw_reba_skeleton(frame, pts, rb)
            draw_reba_table(frame, rb, fps_value)
            ...
            cv2.imshow("REBA Pose Scoring (Host CPU)", frame)
