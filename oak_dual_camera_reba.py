...
def main():
    ...
    try:
        with contextlib.ExitStack() as stack:
            ...
            # Both use USB 2 (HIGH) to avoid bandwidth contention on
            # shared USB hub
            log("Connecting to profile camera...")
            profile_dev = stack.enter_context(
                dai.Device(profile_info, dai.UsbSpeed.HIGH))
            log(f"Profile camera connected: {profile_dev.getDeviceName()} "
                f"(USB: {profile_dev.getUsbSpeed()})")
            time.sleep(2.0)  # Stagger device connections

            log("Connecting to front camera...")
            front_dev = stack.enter_context(
                dai.Device(front_info, dai.UsbSpeed.HIGH))
            log(f"Front camera connected: {front_dev.getDeviceName()} "
                f"(USB: {front_dev.getUsbSpeed()})")
            time.sleep(2.0)

            # Build profile camera pipeline (don't start yet)
            log("Building profile camera pipeline...")
            ...
            # Build front camera pipeline (don't start yet)
            log("Building front camera pipeline...")
            ...
            # Start both pipelines with stagger
            log("Starting profile camera pipeline...")
            profile_pipeline.start()
            log("Profile camera started.")
            time.sleep(2.0)  # XLink stability stagger
